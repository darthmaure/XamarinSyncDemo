...
{
    /// <summary>
    /// Adopted solution for getting actual file path from Andoid-like URI
    /// Source: https://stackoverflow.com/questions/46975574/how-to-get-actual-path-from-uri-xamarin-android
    /// </summary>
    public class FilesLocationService : IFilesLocationService
    {
        public string GetActualPathForFile(object uri, object context)
        {
            return GetActualPath((Uri)uri, (Context)context);
        }
        ...
        private static string GetActualPath(Uri uri, Context context)
        {
        ...
        private static string GetDriveFileAbsolutePath(Context context, Uri uri)
        {
            ICursor cursor = null;
        ...
        private async static Task SaveFileFromUri(Context context, Uri uri, string destinationPath)
        {
        ...
        private static bool IsExternalStorageDocument(Uri uri) => _externalStorageAuthority.Equals(uri.Authority);

        //Whether the Uri authority is DownloadsProvider.
        private static bool IsDownloadsDocument(Uri uri) => _downloadsAuthority.Equals(uri.Authority);

        //Whether the Uri authority is MediaProvider.
        private static bool IsMediaDocument(Uri uri) => _mediaAuthority.Equals(uri.Authority);

        //Whether the Uri authority is Google Photos.
        private static bool IsGooglePhotosUri(Uri uri) => _photoAuthority.Equals(uri.Authority);

        //Whether the Uri authority is Google Disk.
        private static bool IsDiskContentUri(Uri uri) => _diskAuthority.Equals(uri.Authority);

        //Whether the Uri authority is Google Disk Legacy.
        private static bool IsDiskLegacyContentUri(Uri uri) => _diskLegacyAuthority.Equals(uri.Authority);
    }
}